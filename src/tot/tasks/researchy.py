...
from tot.prompts.researchy import * 
...
class ResearchyEnv:
    def __init__(self, file='researchy_questions.test.jsonl'):
        path = os.path.join(DATA_PATH, 'researchy', file)
        with open(path, 'r') as file:
            self.file = [json.loads(line) for line in file]
    ...
    def reset(self, idx, decomp=None, status=None, steps=None):
        ...
        # ground truth values
        # full_answer = self.file[idx]['answer']
        # answer_parts = full_answer.split('####')
        # self.decomp_gt = answer_parts[0].strip()
        # self.ans_gt = answer_parts[1].strip()
        docstream = self.file[idx]['DocStream']
        for url_data in docstream:
            facts = url_data['Snippet']
        self.facts = facts
    ...
    def eval_status(self, answer):
        prompt = eval_score_prompt.format(Question=self.data, ReferenceAnswer='', CandidateAnswer=answer)
        res = (gpt(prompt)[0]).lower()
        print(res)
...
class ResearchyTask(Task):
    ...
    def get_facts(self, idx: int) -> str:
        docstream = self.data[idx]['DocStream']
        for url_data in docstream:
            facts = url_data['Snippet']
        # return facts
        return []
    ...
    def eval_status(self, answer, idx):
        prompt = eval_score_prompt.format(Question=self.get_input(idx), ReferenceAnswer='', CandidateAnswer=answer)
        res = (gpt(prompt)[0]).lower()
        print(res)
